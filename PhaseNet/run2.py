    ...
    with tf.compat.v1.Session(config=sess_config) as sess:
        ...
        for epoch in range(args.epochs):
            progressbar = tqdm(range(0, data_reader.num_data, args.batch_size),
                               desc="{}: epoch {}".format(log_dir.split("/")[-1], epoch))
            for step in progressbar:
                X_batch, Y_batch = sess.run(batch)
                loss_batch = model2.train_on_batch(sess, X_batch, Y_batch, summary_writer, args.drop_rate)
                if epoch < 1:
                ...
                progressbar.set_description(
                    "{}: epoch {}, loss={:.6f}, mean={:.6f}".format(log_dir.split("/")[-1], epoch, loss_batch,
                                                                    mean_loss))
            ...
            if data_reader_valid is not None:
                ...
                for step in progressbar:
                    ...
                    loss_batch, preds_batch = model2.valid_on_batch(sess, X_batch, Y_batch, summary_writer)
                    valid_step += 1
                ...
                loss_batch, preds_batch = model2.valid_on_batch(sess, X_batch, Y_batch, summary_writer)
            # loss_batch, pred_batch, logits_batch, X_batch, Y_batch = model2.train_on_batch(sess, summary_writer, args.drop_rate, raw_data=True)
...
def valid_fn(args, data_reader, figure_dir=None, result_dir=None):
    ...
    model2 = Model(config, input_batch=batch, mode='valid')
    sess_config = tf.compat.v1.ConfigProto()
    sess_config.gpu_options.allow_growth = True
    sess_config.log_device_placement = False
    ...
    with tf.compat.v1.Session(config=sess_config) as sess:
        ...
        for step in progressbar:
            ...
            fname_batch, itp_batch, its_batch = model2.test_on_batch(sess, summary_writer)
            total_step += 1
            mean_loss += (loss_batch - mean_loss) / total_step
...
def pred_fn(args, data_reader, figure_dir=None, result_dir=None, log_dir=None):
    ...
    model2 = Model(config, batch, "pred")
    sess_config = tf.compat.v1.ConfigProto()
    sess_config.gpu_options.allow_growth = True
    sess_config.log_device_placement = False
    ...
    with tf.compat.v1.Session(config=sess_config) as sess:
        ...
        if args.input_mseed:
            ...
            while True:
                ...
                pred_batch, X_batch, fname_batch = sess.run([model2.preds, batch[0], batch[1]],
                                                            feed_dict={model2.drop_rate: 0,
                ...
                for i in range(len(fname_batch)):
                    ...
                    #     breakpoint()
                    row = "{},[{}],[{}],[{}],[{}]".format(fname_batch[i].decode(),
                                                          " ".join(map(str, picks_batch[i][0][0])),
                                                          " ".join(map(str, picks_batch[i][0][1])),
                                                          " ".join(map(str, picks_batch[i][1][0])),
                                                          " ".join(map(str, picks_batch[i][1][1])))
                    fclog.write(row + "\n")
                    picks[fname_batch[i].decode()] = {"itp": picks_batch[i][0][0], "tp_prob": picks_batch[i][0][1],
                                                      "its": picks_batch[i][1][0], "ts_prob": picks_batch[i][1][1]}
            ...
            for step in tqdm(range(0, data_reader.num_data, args.batch_size), desc="Pred"):
                if step + args.batch_size >= data_reader.num_data:
                    ...
                    sess.run(data_reader.queue.close())
                # ---------------------plot mode
                # pred_batch, X_batch, fname_batch = sess.run([model2.convs[plot_depth], batch[0], batch[1]],
                #                                             feed_dict={model2.drop_rate: 0,
                #                                                        model2.is_training: False})
                pred_batch, X_batch, fname_batch = sess.run([model2.preds, batch[0], batch[1]],
                                                            feed_dict={model2.drop_rate: 0,
                ...
                for kk in range(0, y.shape[1]):
                # for kk in range(0, 20):
                    if (max(y[:, kk]) - min(y[:, kk]))!=0:
                        normal_y = (y[:, kk] - min(y[:, kk])) / (max(y[:, kk]) - min(y[:, kk]))
                    else:
                        normal_y = y[:, kk]
                    plt.plot(normal_y + kk*1.1, 'k')
                plt.yticks([])
                plt.savefig(
                    f'/home/jc/work/EqtAndPhaseNet/results_analysis/p4_eqt_process_image/example/depth_{plot_depth}.pdf')
                plt.close()
                # ----------------------predict mode
...
def main(args):
    global plot_depth
    if 'plot_depth' in args:
        plot_depth= args.plot_depth
    else:
        plot_depth = 0
